...
async def read_all_by_user(request: Request, db: Session = Depends(get_db)):
    ...
    if user is None:
        ...
        return templates.TemplateResponse("home.html", {"request": request, "posts": posts, "user": user, "msg": msg})
...
async def add_new_post(request: Request):
    ...
    return templates.TemplateResponse("add-post.html", {"request": request, "user": user, "msg": msg})
...
async def edit_post_commit(request: Request, edit_data: EditModel = Depends(edit_form_data), db: Session = Depends(get_db)):
    ...
    user = await get_current_user(request)
    if user is None:
        return RedirectResponse(url="/auth?msg=Authentication required", status_code=status.HTTP_302_FOUND)
...
async def read_post(request: Request, post_id: int, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("read-post.html", {"request": request, "post": post, "user": user, "msg": post.title+" Read"})
